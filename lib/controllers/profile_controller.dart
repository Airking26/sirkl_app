...
class ProfileController extends GetxController{
  ...
  Future<void> promptClaimUsername(BuildContext context) async =>
      await showDialog(context: context, builder: (context) => AlertDialog(
        ...
        title: const Column(
          children: [
            Text("Claim your free username", style: TextStyle(fontFamily: 'Gilroy', fontSize: 24, fontWeight: FontWeight.w700), textAlign: TextAlign.center,),
            SizedBox(height: 2,),
            Text("Remembering your wallet address is easier with a free username from SIRKL.io", style: TextStyle(fontFamily: 'Gilroy', fontWeight: FontWeight.w500, fontSize: 14), textAlign: TextAlign.center,),
          ],
        ),
        content: Column(
  ...
  Future<void> promptChooseUsername(BuildContext context) async => await showDialog(context: context, builder: (context) => StatefulBuilder(
    builder: (context, setState) => AlertDialog(
      ...
      title: const Column(
        children: [
          Text("Pick your username", style: TextStyle(fontFamily: 'Gilroy', fontSize: 24, fontWeight: FontWeight.w700), textAlign: TextAlign.center,),
          SizedBox(height: 2,),
          Text("Choose wisely! You won't be able to change it afterwards", style: TextStyle(fontFamily: 'Gilroy', fontWeight: FontWeight.w500, fontSize: 14), textAlign: TextAlign.center,),
        ],
      ),
      content: Obx(() => Column(
        mainAxisSize: MainAxisSize.min,
        ...
        children: [
          TextField(
            ...
            onChanged: (value) async {
              ...
              if (value.isNotEmpty) {
                if(!RegExp(r'^[a-z0-9]+$').hasMatch(value)) {
                  ...
                  _errorMessage.value = "Username must contain only lowercase letter and numbers";
                  ...
                  if(await ProfileRepo.checkIsUsernameAvailable(value)){
            ...
            decoration: InputDecoration(
              labelStyle: TextStyle(fontFamily: 'Gilroy', color: MediaQuery.of(context).platformBrightness == Brightness.dark
                  ? Colors.white
                  : Colors.black87),
              labelText: 'Username',
              ...
              enabledBorder: OutlineInputBorder(
                borderRadius: BorderRadius.circular(8.0),
                borderSide: BorderSide(color: _usernameController.text.isEmpty ? Colors.grey:  _isUsernameValid.value ? SColors.activeColor : Colors.redAccent, width: 2.0),
              ),
          ...
          !_isUsernameValid.value && _errorMessage.value.isNotEmpty ?
          Padding(
            padding: const EdgeInsets.only(top: 4.0, left: 4),
            child: Text(
              _errorMessage.value,
              style: const TextStyle(color: Colors.redAccent, fontFamily: 'Gilroy', fontWeight: FontWeight.w500),
            ),
          ) :
          SizedBox(),
        ],
      ...
      actions: <Widget>[
        Obx(() => _isCheckingUsernameValidity.value ? Center(child: CircularProgressIndicator(color: SColors.activeColor,)) : ElevatedButton(
          onPressed: () async {
            if(_usernameController.text.isNotEmpty && _usernameController.text.length > 2 && _usernameController.text.length < 9 && _isUsernameValid.value && !_isCheckingUsernameValidity.value){
              await updateMe(UpdateMeDto(userName: _usernameController.text), StreamChat.of(context).client);
              Get.back();
            }
          },
          style: TextButton.styleFrom(
            padding: const EdgeInsets.symmetric(vertical: 12),
            backgroundColor: _usernameController.text.isNotEmpty && _usernameController.text.length > 2 && _usernameController.text.length < 9 && _isUsernameValid.value && !_isCheckingUsernameValidity.value ? SColors.activeColor : Colors.grey,
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(10),
            ),
          ),
