...
class HomeService extends GetConnect{
  ...
  Future<Response<Map<String, dynamic>>> getNFTsContractAddresses(String wallet) => get("https://deep-index.moralis.io/api/v2/nft/$wallet", headers: {"accept": 'application/json', "X-API-Key": con.moralisApiKey}, query: {"chain": 'eth', "format": 'decimal'});
  Future<Response<Map<String, dynamic>>> getNextNFTsContractAddresses(String wallet, String cursor) => get("https://deep-index.moralis.io/api/v2/nft/$wallet", headers: {"accept": 'application/json', "X-API-Key": con.moralisApiKey}, query: {"chain": 'eth', "format": 'decimal',"cursor": cursor});
