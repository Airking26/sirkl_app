...
class CallRepo {
  ...
  static Future<void> createCall(CallCreationDto callCreationDto) async {
    ...
    Response res = await req.post(url: SUrls.callCreate, body: callCreationDto.toJson());
  }
  
    static Future<void> updateCall(CallModificationDto callModificationDto) async {
    SRequests req = SRequests(SUrls.baseURL);
    Response res = await req.post(url: SUrls.callUpdate, body: callModificationDto.toJson());
  }
  static Future<List<CallDto>> retrieveCalls(String offset) async {
    SRequests req = SRequests(SUrls.baseURL);
    Response res = await req.get(SUrls.callRetrieveByOffset(offset));
    return (res.jsonBody() as List<dynamic>).map((e) => CallDto.fromJson(e)).toList();
  }
  static Future<List<UserDTO>> searchUser(String substring, String offset) async {
          SRequests req = SRequests(SUrls.baseURL);

    Response res = await req.get(SUrls.searchUsersBySubstringOffset(substring, offset));
  ...
  static Future<void> endCall(String id, String channel) async {
    SRequests req = SRequests(SUrls.baseURL);
    Response res = await req.get(SUrls.callEndByIdChannel(id, channel));
  }
  static Future<void> missedCallNotification(String id) async {
          SRequests req = SRequests(SUrls.baseURL);
    Response res = await req.get(SUrls.callMissedCallById(id));
  }
