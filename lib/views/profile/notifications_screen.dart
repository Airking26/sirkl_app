...
class _NotificationScreenState extends State<NotificationScreen> with TickerProviderStateMixin{
  ...
  void initState() {
    ...
    if(!_homeController.userMe.value.hasSBT!) _profileController.index.value = 1;
                  ...
                  MediaQuery.removePadding(
                    ...
                    child: SafeArea(
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (context, index) => buildNotificationSirklClub(context, index),
                          separatorBuilder: (context, index){
                          return Divider(color: MediaQuery.of(context).platformBrightness == Brightness.dark ? const Color(0xFF9BA0A5) : const Color(0xFF828282), thickness: 0.2, endIndent: 20, indent: 20, height: 0,);},
                      ),
                    ),
                  ),
                ],
  ...
  Widget buildNotificationSirklClub(BuildContext context, int index) {
    if(_homeController.userMe.value.hasSBT!) {
      ...
      return Padding(padding: const EdgeInsets.only(right: 8, top: 16, bottom: 8), child: ListTile(
        titleAlignment: ListTileTitleAlignment.top,
        leading: ClipRRect(
          borderRadius: BorderRadius.circular(90),
          child: Image.asset("assets/images/app_icon_rounded.png" , width: 55, height: 55, fit: BoxFit.cover),
    ...
    if(item.type == 0){
      return RichText(
        text: TextSpan(
            ...
            children: [
              ...
              TextSpan(text: " added you in his SIRKL - ${timeago.format(timeSince)}", style:
              TextStyle(fontSize: 16, fontFamily: "Gilroy", fontWeight: FontWeight.w500, color: MediaQuery.of(context).platformBrightness == Brightness.dark ? Colors.white : Colors.black.withOpacity(0.6))
              ),
            ]
        ),
      );
      ...
      return RichText(
        text: TextSpan(
            style: const TextStyle(),
            children: [
              TextSpan(text: "You have added ", style: TextStyle(fontSize: 16, fontFamily: "Gilroy", fontWeight: FontWeight.w500, color:MediaQuery.of(context).platformBrightness == Brightness.dark ? Colors.white : Colors.black.withOpacity(0.6))),
              TextSpan(text: item.username ?? item.wallet , style:  TextStyle(fontSize: 16, fontFamily: "Gilroy", fontWeight: FontWeight.w600, color: SColors.activeColor)),
              TextSpan(text: " in your SIRKL - ${timeago.format(timeSince)}", style: TextStyle(fontSize: 16, fontFamily: "Gilroy", fontWeight: FontWeight.w500, color: MediaQuery.of(context).platformBrightness == Brightness.dark ? Colors.white : Colors.black.withOpacity(0.6))),
            ]
        ),
      );
    } else if(item.type == 4 || item.type == 5 || item.type == 6 || item.type == 7 || item.type == 8){
      return Text(item.message!, style: TextStyle(fontSize: 16, fontFamily: "Gilroy", fontWeight: FontWeight.w500, color:MediaQuery.of(context).platformBrightness == Brightness.dark ? Colors.white : Colors.black.withOpacity(0.6)));
    } else {
      return Container();
    }
  }
