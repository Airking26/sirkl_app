...
class _AddUserToGroupScreenState extends State<AddUserToGroupScreen> {
          ...
          Expanded(
            child: Padding(
                ...
                child: Obx(() => Column(
                      ...
                      children: [
                        _chatController.chipsListAddUsers.isEmpty
                            ? Container()
                            : Column(
                                children: [
                                  Container(
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              Text(
                _chatController.channel.value!.extraData["isGroupPaying"] !=
                        null
                    ? "Send Invite"
                    : "Add User",
                style: TextStyle(
              ...
              _chatController.channel.value!.extraData["isGroupPaying"] != null
                  ? const SizedBox(
                      width: 42,
                      height: 42,
                    )
                      ...
                      onPressed: () async {
                        for (var element in _chatController.chipsListAddUsers) {
                          await _chatController.channel.value
                              ?.addMembers([element.id!]);
                          await _commonController.notifyAddedInGroup(
                              NotificationAddedAdminDto(
                                  idUser: element.id!,
                                  idChannel: _chatController.channel.value!.id!,
  ...
  Widget buildToSendChip(BuildContext context, int index) {
    return Padding(
      ...
      child: InputChip(
          ...
          label: Text(
            _chatController.chipsListAddUsers[index].userName.isNullOrBlank!
                ? "${_chatController.chipsListAddUsers[index].wallet!.substring(0, 10)}..."
                : _chatController.chipsListAddUsers[index].userName!,
  ...
  Widget buildNewMessageTile(BuildContext context, int index, UserDTO item) {
    return ListTile(
        ...
        trailing: InkWell(
          onTap: () async {
            if (_chatController.channel.value!.extraData["isGroupPaying"] !=
                null) {
              showDialog(
                  context: context,
                        ...
                        actions: [
                          ...
                          CupertinoDialogAction(
                            ...
                            onPressed: () async {
                              ...
                              connector.onSessionConnect
                                  .subscribe((args) async {
                                await _web3Controller.sendInviteMethod(
                                    connector,
                                    args,
                                    context,
              ...
              var isPresent = await _chatController.channel.value
                  ?.queryMembers(filter: Filter.equal("id", item.id!));
              if (isPresent!.members.isEmpty) {
                if (!_chatController.chipsListAddUsers
