...
class CreateGroupSecondScreen extends StatefulWidget {
  ...
  @override
  State<CreateGroupSecondScreen> createState() =>
      _CreateGroupSecondScreenState();
}
...
class _CreateGroupSecondScreenState extends State<CreateGroupSecondScreen> {
        ...
        trailing: Checkbox(
          onChanged: (selected) {
            if (item.id.isNullOrBlank!) {
              showToast(
                  context, "Only users from SIRKL can be part of a group");
            } else {
              if (selected!) {
                _chatController.chipsList.add(item);
              } else {
                _chatController.chipsList
                    .removeWhere((element) => element.wallet == item.wallet);
              }
              _chatController.chipsList.refresh();
            }
          },
  ...
  Future<void> sendMessageAsGroup() async {
    ...
    var wallets =
        _chatController.chipsList.map((element) => element.wallet!).toList();
    wallets.add(_homeController.userMe.value.wallet!);
    var members =
        _chatController.chipsList.map((element) => element.id!).toList();
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              TextButton(
                onPressed: () async {
                  if (_chatController.chipsList.isEmpty ||
                      _chatController.chipsList.length < 2) {
                ...
                child: Text(
                  "Done",
                  style: TextStyle(
                      color: _chatController.chipsList.isNotEmpty &&
                              _chatController.chipsList.length >= 2
                          ? SColors.activeColor
                          : Colors.grey,
                      fontFamily: "Gilroy"),
