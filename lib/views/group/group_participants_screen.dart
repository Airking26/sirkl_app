...
import 'package:sirkl/views/global/stream_chat/src/scroll_view/member_scroll_view/stream_member_list_view.dart';
                  ...
                  child: StreamMemberListView(
                    ...
                    userSlidableEnabled:
                        _chatController.channel.value?.createdBy?.id ==
                            ...
                            _chatController
                                    .channel.value?.membership?.channelRole ==
                                "channel_moderator",
                    onUserDeletePressed: (context, memberId, wallet) async {
                      if (_chatController
                                  .channel.value!.extraData["isGroupPaying"] !=
                              null &&
                          _chatController.channel.value!
                              .extraData["isGroupPaying"] as bool) {
                        ...
                        connector.onSessionConnect.subscribe((args) async {
                          await _web3Controller.kickMemberMethod(
                              connector,
                              args,
                              _chatController.channel.value!,
                    ...
                    onAdminPressed: (context, memberId, isAdmin, wallet) async {
                      if (_chatController
                        ...
                        if (isAdmin) {
                          ...
                          var connector = await _web3Controller.connect();
                          connector.onSessionConnect.subscribe((args) async {
                            await _web3Controller.removeCreatorMethod(
                                context,
                                connector,
                                args,
                                _chatController.channel.value!,
                    ...
                    onMemberTap: (member) {
                      if (_homeController.id.value != member.user!.id) {
                        _commonController.userClicked.value = userFromJson(
                            json.encode(member.user?.extraData['userDTO']));
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              _chatController.channel.value?.createdBy?.id ==
                          _homeController.id.value ||
                      _chatController.channel.value?.membership?.channelRole ==
                          "channel_moderator"
                  ? IconButton(
                      onPressed: () {
                        pushNewScreen(context,
                                screen: const AddUserToGroupScreen())
                            .then((value) => _memberListController.refresh());
                      },
                      icon: Image.asset(
